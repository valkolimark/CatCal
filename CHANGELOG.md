...
- `ManageCalendarsView`, reachable from Profile: EventKit calendars grouped by the account they came from, each with a visibility toggle, alongside the Google and Outlook connection cards and their per-calendar toggles.
- Duplicate-source detection: when a directly connected account is already syncing through iOS Settings, the card says so and offers a one-tap "hide its iOS calendars" fix. Deliberately a suggestion rather than an automatic change — some people want both — and it matches conservatively, since EventKit exposes an account *title* rather than a guaranteed address. Eleven tests, including that a same-named account under a different provider must not match.
- An optional "Connect your other calendars" step after Sign in with Apple, with Google, Outlook and Skip. Shown once (`hasSeenCalendarConnect`) and it reuses the same `connect()` path as Manage Calendars rather than a parallel onboarding copy.
